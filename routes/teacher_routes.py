...
def ver_feedbacks():
    if 'user' in session and session['tipo'] == 'professor':
        ...
        if len(alunos_data) >= 3:
            ...
            return render_template('feedbacks_professor.html', feedbacks=feedbacks, dificuldades=dificuldades, plot_respostas_url='/static/images/cluster_plot.png')

        else:
            # Caso não tenha alunos suficientes para clustering
            return render_template('feedbacks_professor.html', feedbacks=feedbacks)
